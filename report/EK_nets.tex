...
We are especially insterested here in the set of EK-model that a certain EK-net can generate. However, we will quickly face size isssues : 

For an EK-net $F: \Delta \rightarrow \mathcal{C}$ and a model $S$ of $\mathcal{C}$, we have an obvious EK-model : $\big<SF, Id\big>$.
...
\begin{exmp}
    ...
    For each set $s$, there are as many functors $R^s_x$ as elements $x$ of $s$. These are our candidate to be EK-models. We still need a natural transformation for each of them. Each time, there is exactly one and is it obvious : the identity.
...
%TODO : better proof
Let us suppose that $\Delta$ has only one element $A$. Consequently, $\Delta$ is isomorphic to a subgroup of $T/I$. The set of the subgroups of $T/I$ is in one-to-one correspondance with the set of $T/I$ EK-Nets with $\Delta$ fixed.
...
\begin{defn}[EK pitch class on T/I]
    Let $\Delta$ be the category drawn in Figure \ref{fig:delta-pitchClassDef}.
    An \textbf{EK pitch class} on T/I is define as an EK-net from $\Delta$ to $T/I$.pair integers $(k,i)$ where $k$ is called the key of the pitch class and $i$ is the pitch of the pitch class.
    \begin{tzcategory}{\caption{$\Delta$ category for EK pitch classes}
        \label{fig:delta-pitchClassDef}}
    \node[scale=1.3] (a) at (0,0){
