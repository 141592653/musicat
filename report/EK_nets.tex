...
\begin{defn}[Constraint solving]
    Let $\rho$ be a constraint of $\text{EKN}_\mathcal{C}$ on a pattern $\Delta$ and let $\big<N,\nu\big> : F \rightarrow F'$ be an EK-homography $\nu : F \rightarrow F'$ where the pattern of $F$ is $\Delta$.
