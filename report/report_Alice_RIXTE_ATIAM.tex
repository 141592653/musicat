...
Consequently, we have two classes of equivalent functors. This gives to the analyst two different tools to analyse a point with two arrows on it. Actually, if we label the two arrows of a point with $T_0$ and $I_k$, we get the $k^{th}$ pitch class, and the natural transformation $T_i$ is a transposition. 
...
\begin{defn}
    ...
    \begin{itemize}
        \item objects of $S(\mathcal{C})$ are the list of objects of $\mathcal{C}$
              %TODO : more formal definitions of labeled definitions
        \item morphisms of  $S(\mathcal{C})$ are labeled permutations $\big<p,l\big> : (x_1,\dots,x_n)\rightarrow (y_1,\dots,y_n)$ where $y_i = l(p_i)x_{p_i}$
    \end{itemize}
\end{defn}
